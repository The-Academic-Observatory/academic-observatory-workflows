...
-- This query creates a list of ISSN-L to Normalised Journal Names. The name chosen for each ISSN-L is the most common occurance
WITH name as (
SELECT
    identifier,
...
FROM(
    SELECT
        ...
        COUNT(*) as count
    FROM {{ source('our_research', 'unpaywall') }}
    WHERE journal_issn_l is not null
    GROUP BY journal_issn_l, journal_name
    ORDER BY journal_issn_l, count DESC)
GROUP BY identifier)

-- The main query, which processes Unpaywall and derived COKI derived OA determinations
SELECT
...
*/
  ...
  ARRAY((SELECT repository_institution FROM UNNEST(oa_locations) WHERE host_type = "repository" AND repository_institution IS NOT NULL)) as repository_names,
  best_oa_location.url_for_landing_page,
  best_oa_location.url_for_pdf

FROM 
  {{ source('our_research', 'unpaywall') }}

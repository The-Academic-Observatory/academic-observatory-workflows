...
class TestOrcidTelescope(ObservatoryTestCase):
        ...
        self.assert_dag_structure(
            {
                ...
                "create_datasets": ["transfer_orcid"],
        ...
        env = ObservatoryEnvironment(
            workflows=[
                Workflow(
                    ...
                    class_name="academic_observatory_workflows.workflows.orcid_telescope.OrcidTelescope",
    ...
    def test_telescope(self):
        ...
        orcid_dataset = env.add_dataset()
        with env.create(task_logging=True):
            workflow = OrcidTelescope(
                dag_id=self.dag_id,
                cloud_workspace=env.cloud_workspace,
                orcid_bucket=orcid_bucket,
                bq_dataset_id=orcid_dataset,
            )
            dag = workflow.make_dag()

            # Add connections
            env.add_connection(Connection(conn_id=workflow.aws_orcid_conn_id, uri="http://:aws-orcid-token@"))
            ...
            with env.create_dag_run(dag, first_execution_date) as dag_run:
                expected_release = OrcidRelease(
                    ...
                    bq_main_table_name=workflow.bq_main_table_name,
                    bq_upsert_table_name=workflow.bq_upsert_table_name,
                    bq_delete_table_name=workflow.bq_delete_table_name,
                ...
                first_release = workflow.make_release(
                    dag_run=env.dag_run,
                    run_id=env.dag_run.run_id,
                    task=MagicMock(task_id="dummy"),
                ...
                with patch(
                    "academic_observatory_workflows.workflows.orcid_telescope.gcs_create_aws_transfer"
                ) as mock_transfer:
                    # Transfer failures should raise an error
                    mock_transfer.side_effect = ([False, 0] for _ in range(workflow.transfer_attempts))
                    with self.assertRaises(AirflowException):
                        env.run_task(workflow.transfer_orcid.__name__)
                    dag.clear(task_ids=[workflow.transfer_orcid.__name__])
                    # Transfer success:
                    mock_transfer.side_effect = ([False, 0], [True, 1])
                    ti = env.run_task(workflow.transfer_orcid.__name__)
                    self.assertEqual(State.SUCCESS, ti.state)
                # Upload the fixtures to the bucket to be picked up by the download
                blob_names = []
                file_paths = []
                for record in OrcidTestRecords.first_run_records:
                    blob_names.append(f"{workflow.orcid_summaries_prefix}/{record['batch']}/{record['orcid']}.xml")
                ...
                with patch(
                    "academic_observatory_workflows.workflows.orcid_telescope.OrcidBatch.missing_records"
                ...
                with patch(
                    "academic_observatory_workflows.workflows.orcid_telescope.OrcidRelease.downloaded_records"
                ...
                with patch(
                    "academic_observatory_workflows.workflows.orcid_telescope.OrcidBatch.expected_records"
                ...
                dag.clear(task_ids=[workflow.transform.__name__])
                ...
                with patch(
                    "academic_observatory_workflows.workflows.orcid_telescope.OrcidRelease.upsert_files"
                ) as mock_upsert_files:
                    mock_upsert_files.return_value = []
                    with self.assertRaisesRegex(ValueError, "Number of blobs"):
                        env.run_task(workflow.bq_load_main_table.__name__)
                dag.clear(task_ids=[workflow.bq_load_main_table.__name__])
                ...
                ti = env.run_task(workflow.add_new_dataset_release.__name__)
                self.assertEqual(State.SUCCESS, ti.state)
                dataset_releases = get_dataset_releases(dag_id=self.dag_id, dataset_id=workflow.api_dataset_id)
            ...
            with env.create_dag_run(dag, second_execution_date) as dag_run:
                ...
                # Check that the expected release is the same as the created release
                second_release = workflow.make_release(
                    dag_run=env.dag_run,
                    run_id=env.dag_run.run_id,
                    task=MagicMock(task_id="dummy"),
                    data_interval_start=pendulum.parse(str(env.dag_run.data_interval_start)),
...
class TestCreateOrcidBatchManifest(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.list_blobs_path = "academic_observatory_workflows.workflows.orcid_telescope.gcs_list_blobs"
    ...
    def test_create_orcid_batch_manifest(self):
        ...
        modification dates"""
        blob1 = MagicMock()
        blob1.name = f"{self.test_batch_str}/blob1"
        blob1.bucket.name = self.bucket_name
        blob1.updated = datetime.datetime(2023, 1, 1)
        blob2 = MagicMock()
        blob2.name = f"{self.test_batch_str}/blob2"
        blob2.bucket.name = self.bucket_name
        blob2.updated = datetime.datetime(2023, 1, 2)
        blob3 = MagicMock()
        blob3.name = f"{self.test_batch_str}/blob3"
        blob3.bucket.name = self.bucket_name
        blob3.updated = datetime.datetime(2022, 12, 31)

        with tempfile.TemporaryDirectory() as tmp_dir:
            transform_dir = os.path.join(tmp_dir, "transform")
            os.mkdir(transform_dir)
            test_batch = OrcidBatch(tmp_dir, transform_dir, self.test_batch_str)
            with patch(self.list_blobs_path, return_value=[blob1, blob2, blob3]):
            ...
            self.assertEqual(rows[0]["bucket_name"], blob1.bucket.name)
            self.assertEqual(rows[0]["blob_name"], blob1.name)
            self.assertEqual(rows[0]["updated"], str(blob1.updated))
            self.assertEqual(rows[1]["bucket_name"], blob2.bucket.name)
            self.assertEqual(rows[1]["blob_name"], blob2.name)
            self.assertEqual(rows[1]["updated"], str(blob2.updated))
